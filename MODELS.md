...
凡例:
- 難易度: ★ 簡単 (1-2ノード) / ★★ 普通 (3-5ノード) / ★★★ やや複雑 (6-9ノード) / ★★★★ 複雑 (10-19ノード) / ★★★★★ かなり複雑 (20+ノード)
- ✅ = 実装済み
...
|------|--------|-------------|----------------|--------|--------------|------|
| 平均ぼかし | `blur_3x3` `blur_5x5` `blur_7x7` | `blur` / `boxFilter` | Pad → Conv | ★ | 3×3 / 5×5 / 7×7 | ✅ |
| ガウシアンぼかし | `gaussian_blur_3x3` `gaussian_blur_5x5` `gaussian_blur_7x7` | `GaussianBlur` | Pad → Conv | ★ | 3×3 / 5×5 / 7×7 | ✅ |
| シャープ化 | `sharpen` | `filter2D` (シャープカーネル) | Pad → Conv → Clip | ★★ | 3×3 | ✅ |
| エンボス | `emboss` | `filter2D` (エンボスカーネル) | Pad → Conv → Add → Clip | ★★ | 3×3 | ✅ |
| Sobel エッジ | `sobel` | `Sobel` | Mul → ReduceSum → Pad → Conv(x) → Conv(y) → Abs → Abs → Add → Clip → Expand | ★★★★ | 3×3 | ✅ |
| Scharr エッジ | `scharr` | `Scharr` | Mul → ReduceSum → Pad → Conv(x) → Conv(y) → Abs → Abs → Add → Div → Clip → Expand | ★★★★ | 3×3 | ✅ |
| Laplacian エッジ | `laplacian` | `Laplacian` | Mul → ReduceSum → Pad → Conv → Abs → Div → Clip → Expand | ★★★ | 3×3 | ✅ |
| Prewitt エッジ | `prewitt` | `filter2D` | Mul → ReduceSum → Pad → Conv(x) → Conv(y) → Abs → Abs → Add → Div → Clip → Expand | ★★★★ | 3×3 | ✅ |
| アンシャープマスク | `unsharp_mask_3x3` `unsharp_mask_5x5` `unsharp_mask_7x7` | `GaussianBlur` + 加算 | Pad → Conv → Sub → Mul → Add → Clip | ★★★ | 3×3 / 5×5 / 7×7 | ✅ |
| エッジ強度 (magnitude) | `edge_magnitude` | `magnitude` | Mul → ReduceSum → Pad → Conv(x) → Conv(y) → Mul → Mul → Add → Sqrt → Div → Clip → Expand | ★★★★ | 3×3 | ✅ |
| 背景ムラ補正 | `bg_normalize_3x3` `bg_normalize_5x5` `bg_normalize_7x7` | — | Pad → Conv(Gaussian) → Sub → Add(0.5) → Clip | ★★ | 3×3 / 5×5 / 7×7 | ✅ |
| Difference of Gaussians | `dog_3x3_5x5` `dog_3x3_7x7` `dog_5x5_7x7` | — | Mul → ReduceSum → Pad → Conv(σ1) → Pad → Conv(σ2) → Sub → Abs → Div → Clip → Expand | ★★★★ | 3×3/5×5, 3×3/7×7, 5×5/7×7 | ✅ |
...
| 処理 | 実装名 | OpenCV 相当 | ONNX ノード構成 | 難易度 | パラメータ | 状態 |
|------|--------|-------------|----------------|--------|-----------|------|
| Harris コーナー | `harris_corner_3x3` `harris_corner_5x5` | `cornerHarris` | Mul → ReduceSum → Pad → Conv(Sobel x,y) → Mul(Ix²,Iy²,IxIy) → AveragePool(窓) → det/trace → MinMax正規化 → Clip → Expand | ★★★★★ | block_size, k | ✅ |
| Shi-Tomasi スコア | `shi_tomasi_3x3` `shi_tomasi_5x5` | `cornerMinEigenVal` | Mul → ReduceSum → Pad → Conv(Sobel x,y) → Mul → AveragePool(窓) → 最小固有値 → Sqrt → MinMax正規化 → Clip → Expand | ★★★★★ | block_size | ✅ |
