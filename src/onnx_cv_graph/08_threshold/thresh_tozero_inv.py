"""逆ゼロ化閾値処理の ONNX グラフ定義.

gray > threshold → 0, それ以外 → gray.
OpenCV の THRESH_TOZERO_INV 相当.
"""
...
class ThreshTozeroInvOp(OnnxGraphOp):
    """逆ゼロ化閾値処理.

    gray > threshold の画素は 0、それ以外は gray 値を維持.
    ノード構成: Mul → ReduceSum → Greater → Not → Cast → Mul → Expand.
    """
    ...
    def build_graph(self) -> GraphProto:
        ...
        # 反転: gray <= threshold の画素を残す
        not_node = helper.make_node("Not", ["mask"], ["inv_mask"])
        cast_node = helper.make_node("Cast", ["inv_mask"], ["mask_f"], to=TensorProto.FLOAT)

        # gray * inv_mask (閾値超過は 0)
        mul2_node = helper.make_node("Mul", ["gray", "mask_f"], ["tozero_inv_1ch"])

        # 3ch 拡張
        expand_node = helper.make_node("Expand", ["tozero_inv_1ch", "expand_shape"], ["output"])
        ...
        return helper.make_graph(
            [mul_node, reduce_node, greater_node, not_node, cast_node, mul2_node, expand_node],
            self.op_name,
