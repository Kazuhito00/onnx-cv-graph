...
class PrewittOp(OnnxGraphOp):
    """Prewitt エッジ検出.

    グレースケール → Pad → Conv(Kx), Conv(Ky) → Abs → Add → Div(6) → Clip → 3ch 拡張.
    Prewitt の最大応答は 1+1+1=3 (片方向) なので合計最大 6. /6 で正規化.
    """
    ...
    def build_graph(self) -> GraphProto:
        ...
        kx = np.array([[-1, 0, 1], [-1, 0, 1], [-1, 0, 1]], dtype=np.float32).reshape(1, 1, 3, 3)
        ky = np.array([[-1, -1, -1], [0, 0, 0], [1, 1, 1]], dtype=np.float32).reshape(1, 1, 3, 3)
        ...
        norm = np.array([6.0], dtype=np.float32)
