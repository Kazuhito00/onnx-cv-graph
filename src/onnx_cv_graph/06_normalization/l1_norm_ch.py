"""チャネル軸 L1 正規化の ONNX グラフ定義.

各ピクセルの RGB 3値を L1 ノルムで除算して正規化する.
正規化後の各ピクセルの L1 ノルム (R+G+B) は 1 になる.
"""
...
class L1NormChOp(OnnxGraphOp):
    """チャネル軸 L1 正規化.

    入力 (N,3,H,W) float32 の各ピクセルについて
    (R,G,B) / (|R| + |G| + |B| + eps) を計算する.
    """
