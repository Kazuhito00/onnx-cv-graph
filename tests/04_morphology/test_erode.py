...
def ensure_models():
    ...
    missing = any(
        not (MODEL_DIR / f"erode_{k}x{k}.onnx").exists() for k in KERNEL_SIZES
    )
    if missing:
...
def session_and_k(request):
    ...
    k = request.param
    sess = ort.InferenceSession(str(MODEL_DIR / f"erode_{k}x{k}.onnx"))
    return sess, k
...
class TestErodeValues:
    ...
    def test_erode_shrinks_bright(self, session_and_k):
        """明るいピクセルが収縮すること (暗い点が拡大)."""
        sess, k = session_and_k
        img = np.ones((1, 3, 16, 16), dtype=np.float32)
        img[:, :, 8, 8] = 0.0  # 中央に1ピクセルの黒点
        out = _run(sess, img)
        # 収縮後、中央周囲のカーネル範囲が 0.0 になる
        pad = k // 2
        for ch in range(3):
            assert out[0, ch, 8, 8] == 0.0
            assert out[0, ch, 8 - pad, 8] == 0.0
            assert out[0, ch, 8, 8 + pad] == 0.0
...
class TestErodeVsOpenCV:
    """OpenCV の erode との比較テスト群."""

    def test_matches_opencv(self, session_and_k):
        """ランダム画像で OpenCV erode との一致を検証 (uint8 ±1 許容)."""
        sess, k = session_and_k
        ...
        cv_out = cv2.erode(hwc_u8, kernel)
