...
def ensure_models():
    missing = any(
        not (MODEL_DIR / f"tophat_{k}x{k}.onnx").exists() for k in KERNEL_SIZES
    )
    if missing:
...
def session_and_k(request):
    k = request.param
    sess = ort.InferenceSession(str(MODEL_DIR / f"tophat_{k}x{k}.onnx"))
    return sess, k
...
class TestTopHatValues:
    ...
    def test_output_non_negative(self, session_and_k):
        """トップハットは常に非負 (input >= opening(input))."""
        sess, k = session_and_k
    ...
    def test_bright_spot_detected(self, session_and_k):
        """暗い背景中の明るい点が検出されること."""
        sess, k = session_and_k
        img = np.full((1, 3, 16, 16), 0.1, dtype=np.float32)
        img[:, :, 8, 8] = 1.0  # 明るい点
        out = _run(sess, img)
        assert out[0, 0, 8, 8] > 0.5


class TestTopHatVsOpenCV:
    def test_matches_opencv(self, session_and_k):
        """OpenCV morphologyEx(TOPHAT) との一致 (uint8 ±1 許容)."""
        sess, k = session_and_k
        ...
        cv_out = cv2.morphologyEx(hwc_u8, cv2.MORPH_TOPHAT, kernel)
