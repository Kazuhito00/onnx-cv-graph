...
def ensure_models():
    ...
    missing = any(
        not (MODEL_DIR / f"closing_{k}x{k}.onnx").exists() for k in KERNEL_SIZES
    )
    if missing:
...
def session_and_k(request):
    ...
    k = request.param
    sess = ort.InferenceSession(str(MODEL_DIR / f"closing_{k}x{k}.onnx"))
    return sess, k
...
class TestClosingValues:
    ...
    def test_fills_small_dark_hole(self, session_and_k):
        """小さい暗い穴が埋められること (closing = dilate → erode)."""
        sess, k = session_and_k
        img = np.ones((1, 3, 16, 16), dtype=np.float32)
        img[:, :, 8, 8] = 0.0  # 1ピクセルの暗い穴
        out = _run(sess, img)
        # 膨張で埋まり、収縮で復元されないので 1.0 のまま
        np.testing.assert_allclose(out, 1.0, atol=1e-6)

    def test_output_ge_input(self, session_and_k):
        """クロージングは常に入力以上の値を返すこと (extensive)."""
        sess, k = session_and_k
...
class TestClosingVsOpenCV:
    """OpenCV の morphologyEx(MORPH_CLOSE) との比較テスト群."""

    def test_matches_opencv(self, session_and_k):
        """ランダム画像で OpenCV closing との一致を検証 (uint8 ±1 許容)."""
        sess, k = session_and_k
        ...
        cv_out = cv2.morphologyEx(hwc_u8, cv2.MORPH_CLOSE, kernel)
