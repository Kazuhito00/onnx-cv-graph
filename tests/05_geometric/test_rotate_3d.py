"""3D 回転 (rotate_3d) モデルのテスト.

黒パディング固定の rotate_3d モデルを検証する.
"""
...
def ensure_models():
    ...
    if not (MODEL_DIR / "rotate_3d.onnx").exists():
...
def session():
    return ort.InferenceSession(str(MODEL_DIR / "rotate_3d.onnx"))


def _run(session, img: np.ndarray,
         angle_x=0.0, angle_y=0.0, angle_z=0.0, zoom=1.0) -> np.ndarray:
...
class TestRotate3dValues:
    ...
    def test_padding_is_black(self, session):
        """範囲外ピクセルが黒 (0) になること."""
        img = np.full((1, 3, 8, 8), 0.8, dtype=np.float32)
        out = _run(session, img, angle_y=60.0)
        assert out.min() < 0.01


class TestRotate3dVsOpenCV:
    ...
    def _build_homography(self, H, W, angle_x, angle_y, angle_z, zoom):
        ...
        # 中心基準の逆マッピング:
        # x_src = f * (rt22 * xs - rt02 * zs) / zs
        # y_src = f * (rt22 * ys - rt12 * zs) / zs
        # where xs = rt00*xd/z + rt01*yd/z + rt02*f
        #       ys = rt10*xd/z + rt11*yd/z + rt12*f
        #       zs = rt20*xd/z + rt21*yd/z + rt22*f
        rt00, rt01, rt02 = Rt[0]
        rt10, rt11, rt12 = Rt[1]
        rt20, rt21, rt22 = Rt[2]
