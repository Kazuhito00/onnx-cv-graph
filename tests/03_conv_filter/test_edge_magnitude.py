...
MODEL_PATH = PROJECT_ROOT / "models" / CATEGORY / "edge_magnitude.onnx"
...
class TestEdgeMagnitudeOutputShape:
...
class TestEdgeMagnitudeValues:
    ...
    def test_magnitude_vs_sobel_abs(self, session):
        """sqrt(Gx²+Gy²) ≤ |Gx|+|Gy| を確認 (三角不等式)."""
        rng = np.random.default_rng(42)
        img = rng.random((1, 3, 16, 16), dtype=np.float32)
        mag = _run(session, img)
        # Sobel の |Gx|+|Gy| と比較 (Sobel モデルが存在する前提)
        sobel_path = PROJECT_ROOT / "models" / CATEGORY / "sobel.onnx"
        if sobel_path.exists():
            sobel_sess = ort.InferenceSession(str(sobel_path))
            sobel_out = sobel_sess.run(None, {"input": img})[0]
            # magnitude ≤ sobel (|Gx|+|Gy|) — Clip の影響で厳密ではないが概ね成立
            assert mag.mean() <= sobel_out.mean() + 0.01
