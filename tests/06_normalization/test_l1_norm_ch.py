...
MODEL_PATH = PROJECT_ROOT / "models" / CATEGORY / "l1_norm_ch.onnx"
...
def _numpy_l1_norm_ch(img: np.ndarray) -> np.ndarray:
    """NumPy 参照実装: チャネル軸のみで L1 正規化."""
    l1 = np.abs(img).sum(axis=1, keepdims=True)
    result = img / (l1 + 1e-8)
    return np.clip(result, 0.0, 1.0)


class TestL1NormChOutputShape:
    """出力テンソルの形状を検証するテスト群."""
...
class TestL1NormChValues:
    ...
    def test_per_pixel_l1_norm(self, session):
        """各ピクセルの L1 ノルムが 1 に近いこと."""
        rng = np.random.default_rng(42)
        img = rng.random((1, 3, 8, 8), dtype=np.float32)
        out = _run(session, img)
        # 各ピクセルの L1 ノルム (axis=1)
        l1_per_pixel = np.abs(out).sum(axis=1)
        np.testing.assert_allclose(l1_per_pixel, 1.0, atol=1e-4)

    def test_uniform_image(self, session):
        """均一画像で各チャネルが 1/3 になること."""
    ...
    def test_matches_numpy_reference(self, session):
        ...
        rng = np.random.default_rng(42)
        img = rng.random((2, 3, 16, 16), dtype=np.float32)
        expected = _numpy_l1_norm_ch(img)
        out = _run(session, img)
