...
MODEL_PATH = PROJECT_ROOT / "models" / CATEGORY / "l2_norm.onnx"
...
def _numpy_l2_norm(img: np.ndarray) -> np.ndarray:
    """NumPy 参照実装."""
    l2 = np.sqrt((img ** 2).sum(axis=(1, 2, 3), keepdims=True))
    result = img / (l2 + 1e-8)
    return np.clip(result, 0.0, 1.0)


class TestL2NormOutputShape:
    """出力テンソルの形状を検証するテスト群."""
...
class TestL2NormValues:
    ...
    def test_unit_l2_norm(self, session):
        """出力の L2 ノルムが 1 に近いこと."""
        rng = np.random.default_rng(42)
        img = rng.random((1, 3, 8, 8), dtype=np.float32)
        out = _run(session, img)
        l2 = np.sqrt((out ** 2).sum())
