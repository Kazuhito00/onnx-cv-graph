...
MODEL_PATH = PROJECT_ROOT / "models" / CATEGORY / "l1_norm.onnx"
...
def _numpy_l1_norm(img: np.ndarray) -> np.ndarray:
    """NumPy 参照実装."""
    l1 = np.abs(img).sum(axis=(1, 2, 3), keepdims=True)
    result = img / (l1 + 1e-8)
    return np.clip(result, 0.0, 1.0)


class TestL1NormOutputShape:
    """出力テンソルの形状を検証するテスト群."""
...
class TestL1NormValues:
    ...
    def test_unit_l1_norm(self, session):
        """出力の L1 ノルムが 1 に近いこと."""
        rng = np.random.default_rng(42)
